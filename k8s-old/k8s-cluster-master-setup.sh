...
sudo tar Cxzvf /opt/cni/bin cni-plugins-linux-amd64-${CNI_PLUGIN_VERSION}.tgz
...
sudo cp /etc/containerd/config.toml /etc/containerd/config.toml.bak && sudo rm /etc/containerd/config.toml
...
## kubeadm init (worker olanlarda çalıştırılmayacak)
sudo kubeadm init
...
# sudo kubeadm join 192.168.1.201:6443 --token 25u4p7.lwn2ke2smyl4rsd7 \
# 	--discovery-token-ca-cert-hash sha256:39cdd857e064773c3930dbeb1ea607bc28fb2cb9cff8338fd05f76bb9bbce8cc 
